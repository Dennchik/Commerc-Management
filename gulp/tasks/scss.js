...
export function scss() {
	return $.gulp.src($.path.scss.src)
		...
		.pipe($.gulpIf($.app.isDev, $.debug({ title: '(INIT Source-maps)' })))
		.pipe(
			sass.sync({
				...
				console.error('Sass error:', err.messageFormatted);
				this.emit('end');
			})
		)
		.pipe(postcss([sortMediaQueries()]))
		.pipe($.gulpIf($.app.isProd, autoprefixer($.app.autoprefixer)))
		.pipe($.gulpIf($.app.isProd, $.debug({ title: '(Autoprefixer)' })))
		.pipe($.gulpIf($.app.isProd, $.gulp.dest($.path.scss.dest)))
		.pipe($.gulpIf($.app.isProd, $.size({ title: 'До сжатия - (CSS):' })))
		.pipe(csso($.app.csso))
		.pipe(rename($.app.renameScss))
		.pipe($.gulpIf($.app.isProd, $.size({ title: 'После сжатия - (CSS):' })))
		.pipe($.gulpIf($.app.isDev, $.sourcemaps.write('.')))
		.pipe($.gulpIf($.app.isDev, $.debug({ title: ' (WRITE Source-Maps)' })))
